...
class SettingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($search){
            $query = $query->whereAny(['key'], 'like', "%{$search}%");
        }
    ...
    public function bulk(Request $request)
    {
        ...
        if (is_array($reqs)) {
            ...
            foreach ($reqs as $req) {
                ...
                if (array_key_exists('id', $req) && (array_key_exists('key', $req)) && (array_key_exists('value', $req))) {
                    $query = Setting::findOrFail($req['id']);
                    $query->update($req);

                    $data = Setting::findOrFail($req['id']);
                    array_push($datas, $data);
                }
            }

            if($datas) {
                return new ApiResource(true, 201, 'Update data successfull', $datas, []);
            } else {
                return new ApiResource(false, 400, 'Failed to update data', [], []);
            }
        } else {
            return new ApiResource(false, 400, 'Failed to update data, data not array', [], []);
        }
